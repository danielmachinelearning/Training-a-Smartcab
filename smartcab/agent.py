...
class LearningAgent(Agent):
    ...
    def reset(self, destination=None, testing=False):
        ...
        time_constant = 0.005 * self.iteration 
        self.epsilon = math.cos(time_constant)
        self.alpha = math.cos(time_constant)
        self.iteration += 1
        if testing == True:
            self.epsilon = 0
            self.alpha = 0
    ...
    def createQ(self, state):
        ...
        if self.learning == True:
            if state not in self.Q.keys():
                self.Q[state] = dict() 
                for given_action in self.valid_actions:
                    self.Q[state][given_action] = 0.0
    ...
    def learn(self, state, action, reward):
        ...
        if self.learning == True:
            self.Q[state][action] = \
                self.Q[state][action] + \
                (1-self.alpha)*(self.alpha * (reward + 0))
